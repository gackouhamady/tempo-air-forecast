...
from src.config import PROCESSED_DIR, MODELS_DIR, ALERT_AQI_THRESHOLD
...
def forecast(req: ForecastRequest):
    ...
    X = df.select_dtypes(include=["number"]).drop(columns=[c for c in df.columns if c == "y_next_24h"], errors="ignore")
