...
from src.config import DEFAULT_LAT, DEFAULT_LON, ALERT_AQI_THRESHOLD, PROCESSED_DIR
...
if not feats.empty:
    ...
    if cols:
        st.plotly_chart(line_chart(feats.tail(72), cols, title="Recent Air Quality & Features"), use_container_width=True)

if st.button("Generate Forecast"):
    resp = requests.post("http://localhost:8000/forecast", json={"lat": lat, "lon": lon, "horizon_hours": horizon}, timeout=15)
    if resp.ok:
        ...
        if data:
            ...
            st.plotly_chart(line_chart(dfp, ["forecast"], title="Forecast"), use_container_width=True)
            if dfp["alert"].any():
                st.error(f"⚠️ Air quality forecast exceeds threshold ({ALERT_AQI_THRESHOLD}). Consider limiting outdoor activities.")
            else:
